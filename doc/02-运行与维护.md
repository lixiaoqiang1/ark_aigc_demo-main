...
```
scripts/
├── stop-services.bat      关闭 3000 + 3001（前端+服务端）
├── start-services.bat     新窗口启动前后端
├── restart-services.bat   先关再启前后端（改配置后推荐）
├── stop-server.bat        仅关闭服务端 3001
├── start-server.bat       仅启动服务端 3001
└── restart-server.bat     仅重启服务端 3001
```
