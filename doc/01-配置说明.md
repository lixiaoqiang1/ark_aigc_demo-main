...
| 文件路径 | 谁读取 | 是否必改 | 作用 |
|----------|--------|----------|------|
| **`Server/scenes/Custom.json`** | Node 服务端 | **是** | AK/SK、RTC、ASR、TTS、LLM 全部业务凭证 |
| `Server/scenes/其他.json` | Node 服务端 | 否 | 新增场景时复制 Custom.json |
...
| 服务端 | 端口 | 配置位置 |
|--------|------|----------|
| **`Server/`（默认）** | 3001 | `Server/scenes/*.json` |
| `server_python/` | 3001 | `server_python/scenes/*.json` |
| `rag_llm_server/` | 3001 | `.env` + 代码硬编码 |

同一时间只能启动一个，都会占用 **3001** 端口。
...
1. 复制 `Custom.json` → `Server/scenes/你的场景名.json`
2. 重启服务端（或等 nodemon 自动重启）
3. 前端会自动读取新场景
...
flowchart LR
    ...
    FE -->|POST getScenes| Node
    FE -->|POST StartVoiceChat| Node
    Node -->|AK/SK 签名| Volc
    FE -->|RTC SDK 音视频| Volc
```
...
1. 服务端 nodemon 会自动重启（改 JSON / js 均监听）
