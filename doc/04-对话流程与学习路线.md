...
```mermaid
flowchart LR
    A[Custom.json] --> B[main.py]
    B --> C[token_builder.py]
    C --> D[火山文档]
    D --> E[useCommon.ts]
```
...
| 2 | `main.py` 两个 API | 1 h |
| 3 | `token_builder.py` Token 机制 | 30 min |
...
| 按钮 action | startAgent 是 HTTP 调 Python 服务端 |
